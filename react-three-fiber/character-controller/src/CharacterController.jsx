...
import warriorUrl from "./assets/warrior.glb?url";

const MODEL_URL = warriorUrl;
...
function Character({ groupRef }) {
  const { scene, animations, fit } = useModel(MODEL_URL, {
    targetHeight: 1.8,
    inPlace: true,
  });
  ...
  useFrame((_, rawDelta) => {
    ...
    if (dir.current.lengthSq() > 0) {
      ...
      g.position.addScaledVector(
        dir.current,
        (running ? RUN_SPEED : WALK_SPEED) * delta,
      );
...
export default function CharacterController() {
  return (
    <div style={{ position: "absolute", inset: 0, background: "#87ceeb" }}>
      <Canvas
        shadows
        camera={{ fov: 55, near: 0.1, far: 500, position: [0, 2.2, 4.5] }}
      >
        <Scene />
      </Canvas>
      <div style={hintStyle}>
        WASD move &middot; mouse orbit &middot; Shift run &middot; F dance
      </div>
    </div>
  );
}
