...
const MIME = {
  ...
  '.bin': 'application/octet-stream', '.mp3': 'audio/mpeg', '.wasm': 'application/wasm', '.ico': 'image/x-icon',
}
...
for (const proj of selected) {
  ...
  try {
    ...
    await page.screenshot({ path: join(thumbsDir, `${proj.id}.jpg`), type: 'jpeg', quality: 80 })
