      ...
      <div style={styles.filters}>
        {categories.map((c) => (
          <button
            key={c}
            onClick={() => setFilter(c)}
            style={{ ...styles.filterBtn, ...(filter === c ? styles.filterActive : null) }}
          >
        ...
        {shown.map((p) => (
          <Link key={p.id} to={`/view/${p.id}`} style={styles.card}>
            <div style={styles.thumbWrap}>
              <img src={`${base}thumbnails/${p.id}.jpg`} alt={p.title} loading="lazy" style={styles.thumb} />
              <span style={styles.kind}>{p.category}</span>
            </div>
            <div style={styles.cardBody}>
              <h3 style={{ margin: '0 0 6px', fontSize: 16 }}>{p.title}</h3>
              <p style={{ margin: 0, color: 'var(--muted)', fontSize: 13, lineHeight: 1.4 }}>{p.blurb}</p>
            </div>
          </Link>
        ))}
      </div>
...
const styles = {
  ...
  filterActive: { background: 'var(--accent)', color: '#06232b', borderColor: 'var(--accent)' },
  ...
  thumbWrap: { position: 'relative', aspectRatio: '16 / 10', background: '#0a0a0e' },
