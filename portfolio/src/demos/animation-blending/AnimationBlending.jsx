...
import { OrbitControls, Grid, Environment, useAnimations } from '@react-three/drei'
...
function Model({ base, weights, speed }) {
  ...
  const clips = useMemo(() => {
    ...
    return animations.map((clip) => {
      ...
      additive.tracks = additive.tracks.filter((t) => !t.name.endsWith('.position'))
  ...
  useEffect(() => {
    ...
    for (const name of BASE_STATES) {
      ...
      if (action) action.reset().setEffectiveWeight(name === 'idle' ? 1 : 0).play()
    }
  ...
  useEffect(() => {
    if (!actions) return
    for (const { name } of ADDITIVE) actions[name]?.setEffectiveWeight(weights[name] ?? 0)
  }, [weights, actions])
...
function Scene({ base, weights, speed }) {
  return (
    <>
      ...
      <OrbitControls target={[0, 1, 0]} enablePan={false} minDistance={1.8} maxDistance={8} maxPolarAngle={Math.PI * 0.5} />
    </>
  )
}
...
export default function AnimationBlending() {
  ...
  const [weights, setWeights] = useState({ agree: 0, headShake: 0, sad_pose: 0, sneak_pose: 0 })
  const [speed, setSpeed] = useState(1)

  return (
    <div style={{ position: 'absolute', inset: 0, background: '#0e0e12' }}>
      <Canvas shadows camera={{ fov: 50, near: 0.1, far: 100, position: [0, 1.4, 3.6] }}>
          ...
          {ADDITIVE.map(({ name, label }) => (
            <label key={name} style={sliderRow}>
              ...
              <input
                ...
                value={weights[name]}
                onChange={(e) =>
                  setWeights((w) => ({ ...w, [name]: parseFloat(e.target.value) }))
                }
                style={{ flex: 1 }}
              />
            </label>
...
const sliderRow = { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }
const segButton = {
  flex: 1,
  background: '#16161d',
  border: '1px solid #26262f',
