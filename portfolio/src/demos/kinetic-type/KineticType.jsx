...
function KineticMesh({ text, geometry, scrollSpeed, spin, repeat, ripple, color }) {
      ...
      <div style={panelStyle}>
        ...
        <Section label="Text color">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <input type="color" value={color} onChange={(e) => setColor(e.target.value)} style={colorStyle} />
        ...
        <Section label="Shape">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {GEOMETRIES.map((g) => (
              <button key={g} onClick={() => setGeometry(g)} style={{ ...segButton, ...(geometry === g ? segActive : null) }}>
                {g}
              </button>
            ))}
          </div>
        ...
        <Slider label={`Scroll ${scrollSpeed.toFixed(2)}`} min={-1} max={1} step={0.01} value={scrollSpeed} onChange={setScrollSpeed} />
        <Slider label={`Spin ${spin.toFixed(2)}`} min={-1} max={1} step={0.01} value={spin} onChange={setSpin} />
        <Slider label={`Density ${repeat}`} min={1} max={8} step={1} value={repeat} onChange={setRepeat} />
        <Slider label={`Ripple ${ripple.toFixed(3)}`} min={0} max={0.06} step={0.001} value={ripple} onChange={setRipple} />
      </div>
    </div>
  )
}
...
const labelStyle = { fontSize: 12, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 1 }
const inputStyle = {
  background: '#0e0e12',
  border: '1px solid #26262f',
  borderRadius: 8,
...
const colorStyle = { width: 44, height: 30, background: 'none', border: '1px solid #26262f', borderRadius: 6, cursor: 'pointer' }
const segButton = {
  background: '#16161d',
  border: '1px solid #26262f',
  color: 'var(--text)',
...
const segActive = { background: 'var(--accent)', color: '#06232b', borderColor: 'var(--accent)' }
