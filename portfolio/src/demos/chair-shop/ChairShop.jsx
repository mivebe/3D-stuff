...
const CHAIRS = [
  {
    url: '/chairs/armchairYellow.gltf',
    bg: '#f15946',
    title: ['Meet the new', 'shopping experience', 'for online chairs'],
  },
  {
    url: '/chairs/armchairGreen.gltf',
    bg: '#571ec1',
    title: ['And we even', 'got different colors'],
  },
  {
    url: '/chairs/armchairGray.gltf',
    bg: '#636567',
    title: ['And yes', 'we even got', 'monochrome'],
  },
]

CHAIRS.forEach((c) => useGLTF.preload(c.url))

function Chair({ url, index }) {
      ...
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        ...
        <Suspense fallback={null}>
          <ScrollControls pages={CHAIRS.length} damping={0.25}>
            ...
            <Scroll>
              {CHAIRS.map((c, i) => (
                <Chair key={c.url} url={c.url} index={i} />
              ))}
            </Scroll>
            <Scroll html style={{ width: '100%' }}>
              {CHAIRS.map((c, i) => (
                <div key={c.url} style={{ ...sectionStyle, top: `${i * 100 + 16}vh` }}>
                  {c.title.map((line, li) => (
