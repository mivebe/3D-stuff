...
export default class Room {
  ...
  setSpawn() {
    ...
    if (tv) {
      ...
      pos = new THREE.Vector3(t.x, this.floorY, t.z).add(
        toCenter.multiplyScalar(2.6),
      );
    }
    ...
    const m = 0.4;
    pos.x = Math.max(
      this.floorBounds.min.x + m,
      Math.min(this.floorBounds.max.x - m, pos.x),
    );
    pos.z = Math.max(
      this.floorBounds.min.z + m,
      Math.min(this.floorBounds.max.z - m, pos.z),
    );
  ...
  setLivingRoomDoors() {
    ...
    if (joystick)
      this.registerDoor(joystick, Y, -Math.PI / 2, "cabinet", "world");

    for (let i = 1; i <= 9; i++) {
      const node = this.livingRoom.getObjectByName(
        `Door_Upper_${String(i).padStart(2, "0")}`,
      );
  ...
  registerDoor(node, axis, angle, label, space) {
    ...
    node.userData.interactive = {
      prompt: () =>
        node.userData.door.isOpen ? `close ${label}` : `open ${label}`,
  ...
  pushWallColliders(box, openings, head) {
    ...
    for (const o of openings) {
      if (
        o.max[thin] < box.min[thin] - 0.05 ||
        o.min[thin] > box.max[thin] + 0.05
      )
        continue;
      if (o.max[axis] <= box.min[axis] || o.min[axis] >= box.max[axis])
        continue;
      if (o.min.y > head) continue;
      cuts.push([
        Math.max(box.min[axis], o.min[axis]),
        Math.min(box.max[axis], o.max[axis]),
      ]);
    }
    if (!cuts.length) {
